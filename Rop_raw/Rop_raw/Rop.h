...
class Ropperdis {
 ...
 private:
   std::multiset<Gadget*> Ropperdis::find_gadget_in_memory(const unsigned char* data, unsigned long long size, unsigned long long vaddr, uint32_t m_depth = 3);
  std::multiset<Gadget*>find_all_gadget_from_ret(const unsigned char* data, unsigned long long vaddr, const DISASM* ending_instr_disasm, unsigned int len_ending_instr);
